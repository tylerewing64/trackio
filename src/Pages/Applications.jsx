...
function Applications() {
  ...
  useEffect(()=> {
    async function fn () { 
      const response =  await getApplicationsW_user_id(Number(Cookies.get('id')));
  ...
  > 
     ...
     <input type = 'text' className='font border-b border-gray-600 outline-none  text-sm' name = 'job_title' onChange= {() => setErrorMsg('')} />
     <span className='text-xs italic mt-2'>Company*</span>
     <input type = 'text' className='font border-b border-gray-600 outline-none text-sm' name = 'job_company' onChange= {() => setErrorMsg('')} />
     <span className='text-xs italic mt-2'>Location*</span>
     <input type = 'text' className='font border-b border-gray-600 outline-none text-sm' name = 'job_location'onChange= {() => setErrorMsg('')}  />
     <span className='text-xs italic mt-2'>URL*</span>
     <input type = 'text' className='font border-b border-gray-600 outline-none text-sm' name = 'job_posting_url' onChange= {() => setErrorMsg('')} />
     <span className='text-xs italic mt-2'>Description</span>
     <input type = 'text' className='font border-b border-gray-600 outline-none text-sm' name = 'job_description' />
    ...
    <div className='mt-5 text-xs'>
      <div className='flex w-full items-center mb-1 cursor-pointer'>
        <AddRoundedIcon/>
        <button className='ml-1 ' onClick= {()=> setToggleApplicationPopup(true)} >New Application</button>
